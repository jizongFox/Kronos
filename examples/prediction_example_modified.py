...
def plot_prediction(kline_df, pred_df, pred_std=None):
    ...
    if pred_std is not None and 'close' in pred_std.columns:
        ...
        ax1.fill_between(pred_df.index, lower, upper, color='red', alpha=0.2, label='±2σ')
    ...
    if pred_std is not None and 'volume' in pred_std.columns:
        ...
        ax2.fill_between(pred_df.index, v_lower, v_upper, color='red', alpha=0.2, label='±2σ')
...
df2 = yf.download("btc-usd", start="2025-07-23", interval="1h", auto_adjust=True, progress=False,
...
y_timestamp = pd.Series(pd.date_range(start=y_timestamp_start, end=y_timestamp_end, freq="h"))
...
preds = []
for _ in range(30):
    pred_df = predictor.predict(df=x_df, x_timestamp=x_timestamp, y_timestamp=y_timestamp, pred_len=pred_len, T=1.0,
                                top_p=0.95, sample_count=1, verbose=True)
