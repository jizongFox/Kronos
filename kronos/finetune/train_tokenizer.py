...
from kronos.finetune.utils.training_utils import (
    setup_ddp,
    cleanup_ddp,
    set_seed,
    get_model_size,
...
def create_dataloaders(config: dict, rank: int, world_size: int):
    ...
    print(
        f"[Rank {rank}] Train dataset size: {len(train_dataset)}, Validation dataset size: {len(valid_dataset)}"
    )
    ...
    print(
        f"[Rank {rank}] Dataloaders created. Train steps/epoch: {len(train_loader)}, Val steps: {len(val_loader)}"
    )
    ...
    for epoch_idx in range(config["epochs"]):
        ...
        for i, (ori_batch_x, _) in enumerate(train_loader):
            ...
            for j in range(config["accumulation_steps"]):
                ...
                end_idx = (j + 1) * (
                    ori_batch_x.shape[0] // config["accumulation_steps"]
                )
            ...
            if rank == 0 and logger:
                avg_loss = current_batch_total_loss / config["accumulation_steps"]
                logger.log_metric(
                    "train_tokenizer_loss_batch", avg_loss, step=batch_idx_global_train
                )
                logger.log_metric(
                    f"train_vqvae_vq_loss_each_batch",
                    bsq_loss.item(),
                    step=batch_idx_global_train,
                )
                logger.log_metric(
                    f"train_recon_loss_pre_each_batch",
                    recon_loss_pre.item(),
                    step=batch_idx_global_train,
                )
                logger.log_metric(
                    f"train_recon_loss_each_batch",
                    recon_loss_all.item(),
                    step=batch_idx_global_train,
                )
                logger.log_metric(
        ...
        if rank == 0:
            ...
            if logger:
                logger.log_metric(
                    "val_tokenizer_loss_epoch", avg_val_loss, epoch=epoch_idx
                )
