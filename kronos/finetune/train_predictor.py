...
from kronos.model.kronos import KronosTokenizer, Kronos
...
def train_model(model, tokenizer, device, config, save_dir, logger, rank, world_size):
    ...
    if rank == 0:
        ...
        print(
            f"Effective BATCHSIZE per GPU: {config['batch_size']}, Total: {effective_bs}"
        )
    ...
    for epoch_idx in range(config["epochs"]):
        ...
        for i, (batch_x, batch_x_stamp) in enumerate(train_loader):
            ...
            if rank == 0 and logger:
                lr = optimizer.param_groups[0]["lr"]
                logger.log_metric(
                    "train_predictor_loss_batch", loss.item(), step=batch_idx_global
                )
                logger.log_metric(
                    "train_S1_loss_each_batch", s1_loss.item(), step=batch_idx_global
                )
                logger.log_metric(
                    "train_S2_loss_each_batch", s2_loss.item(), step=batch_idx_global
                )
        ...
        if rank == 0:
            ...
            if logger:
                logger.log_metric(
                    "val_predictor_loss_epoch", avg_val_loss, epoch=epoch_idx
                )
